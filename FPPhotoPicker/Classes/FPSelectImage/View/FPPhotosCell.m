...
//static NSString *const FPPhotosCollectionCellDeselectImageName = @"FPPhotos.bundle/FP_deselect";

@interface FPPhotosCell ()
...
- (void)photosCellWillLoad
{
    ...
    self.shadeView = ({
        ...
        [view addTarget:self action:@selector(shadeViewDidPress:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (FP_iOS_Version_GreaterThan(9.1)) {
...
- (void)addChooseControl
{
    ...
    /// normal
    [_chooseButton setImage:/*FPPhotosCollectionCellDeselectImageName.FP_image*/NSBundle.fp_deselect forState:UIControlStateNormal];
...
#pragma mark - 点击事件s
// 不能选择的蒙层
- (void)shadeViewDidPress:(UIControl *)sender{
...
- (void)chooseButtonDidTap:(id)sender
{
    ...
    if (self.actionTarget && [self.actionTarget respondsToSelector:@selector(photosCellDidTouchUpInSlide:asset:indexPath:complete:)]) {
        ...
        [self.actionTarget photosCellDidTouchUpInSlide:self asset:self.asset indexPath:self.indexPath complete:^(FPPhotosCellAnimatedStatus status, BOOL selected,NSUInteger count) {
            
            if (status == FPPhotosCellAnimatedStatusPermit) {//允许使用动画
